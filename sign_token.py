...
def sign_token(private_key, blinded_request):
    ...
    # Create simple DB for POH
    # fill it
    # Check that public key is  in POH list
    # remove the right for POH

    # blinded_request = int(sys.argv[2])
    blinded_request = [blinded_request, get_y_coordinate(blinded_request)]
    blinded_token = ec_mult(
        private_key, blinded_request, ALPHA, FIELD_PRIME)[0]
    #v = get_random_private_key()
    v = 50
    vG = ec_mult(v, EC_GEN, ALPHA, FIELD_PRIME)[0]
    vH = ec_mult(v, blinded_request, ALPHA, FIELD_PRIME)[0]
    c = pedersen_hash(pedersen_hash(pedersen_hash(vG, vH), pedersen_hash(
        public_key, blinded_token)), pedersen_hash(EC_GEN[0], blinded_request[0]))
