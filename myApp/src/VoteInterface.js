...
function callEndVotingPhase(resultat) {
  ...
  axios({
    method: "post",
    url: "http://172.17.0.3:5000/api/end_voting_phase",
    data: {
      message: "vitalik<3",
    },
  })
...
function callResultat(resultat) {
  axios({
    method: "get",
    url: "http://172.17.0.2:4242/api/get_result",
  })
    .then((response) => {
      console.log(response);
...
function callVote(resultat) {
  ...
  axios({
    method: "post",
    url: "http://172.17.0.3:5000/api/vote",
    data: {
      vote: resultat,
...
function ToggleGroup() {
  return (
    <div>
      <button className={"btn-grad2"} onClick={() => callVote("Yes")}>
        YES
      </button>
      <button className={"btn-grad2"} onClick={() => callVote("No")}>
        NO
      </button>
